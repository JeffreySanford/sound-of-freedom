...
describe('SongsController', () => {
  ...
  const mockOrchestrator = { submitJob: jest.fn(), generateSong: jest.fn() } as any;
  ...
  const mockJobsService = { createJob: jest.fn() } as any;
  ...
  it('should call ollama service', async () => {
    ...
    const maybe = await controller.generateMetadata(
      {} as any,
      {
        narrative: 'x',
        duration: 30,
    ...
    const res = maybe && typeof (maybe as any).subscribe === 'function' ? await firstValueFrom(maybe as any) : maybe;
