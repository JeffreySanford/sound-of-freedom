...
- Orchestrator: Node-based service with Redis as job queue. Implements `POST /jobs` -> returns `jobId`, `GET /jobs/:id` -> read job details. Adds `/metrics`
endpoint via `prom-client` for Prometheus.
...
- [ ] 11. Security hardening: JWT validation for job ownership; secret management for model keys & object storage
- Status: In Progress — JWT validation added on JobsGateway connect; server will now verify token and only allow job subscriptions for owners or admins. Orchestrator
and worker reporting remain behind `POST /jobs/report` endpoint (protected separately).
