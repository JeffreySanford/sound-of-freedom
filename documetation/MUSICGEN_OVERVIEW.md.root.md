...
This document summarizes best practices, OS choices, model & dataset sources, and deployment guidance for MusicGen-based
inference in the `sound-creator` platform.

Summary

- MusicGen is a state-of-the-art music generation model (research code in `audiocraft`), capable of producing audio
  given prompts and conditioning. Learn more: <https://github.com/facebookresearch/audiocraft>
...
- MusicGen is a music generation model (research code in `audiocraft`) capable of producing audio given prompts and
  conditioning. Learn more: <https://github.com/facebookresearch/audiocraft>
...
- Hugging Face: search for `musicgen` — examples include `facebook/musicgen-small`, `facebook/musicgen-medium`, and
...
- Do not bake model weights into a Docker image during build; instead, fetch models at runtime or mount a shared model
  volume.
...
- For large models, confirm whether the model weights are allowed to be redistributed. If the license disallows
  redistribution, download the model at runtime from the host or a permitted storage location.
