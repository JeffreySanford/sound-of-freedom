...
* **model\_artifacts**: One doc per model snapshot (path, version, checksums, license metadata, source\_url, size, tags).
* **datasets**: Dataset manifests and provenance records.
* **licenses**: Snapshot of license text files and parsed license metadata (spdx, commercial\_use boolean, notes).
* **checksums**: Records of computed checksums per artifact and audit history (useful for re-verification).
* **inventory\_versions**: Manifest versions referencing sets of `model_artifacts` and `datasets` (immutable once published).
